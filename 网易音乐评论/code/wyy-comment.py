...
def hotcomments(html, songname, i, pages, total):
    ...
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            all_info.append([user['nickname'], item['content']])
            # 回复评论
            if len(item['beReplied']) != 0:
                for reply in item['beReplied']:
                    ...
                    all_info.append([replyuser['nickname'], reply['content']])
            m += 1
...
def comments(html, songname, i, pages, total):
    ...
    for item in html['comments']:
        ...
        all_info.append([user['nickname'], item['content'], item['likedCount']])
        # 回复评论
        if len(item['beReplied']) != 0:
            for reply in item['beReplied']:
                # 提取发表回复评论的用户名
                replyuser = reply['user']
                print("回复：{} : {}".format(replyuser['nickname'], reply['content']))
                all_info.append([replyuser['nickname'], reply['content'], None])
        j += 1
...
if __name__ == "__main__":
    ...
    ha_list = [["水星记", 441491828], ['小城夏天', 1934251776], ['孤勇者', 1901371647], ['Slow Down', 1356658022]]
